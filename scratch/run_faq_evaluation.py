...
def run_evaluation():
    print(f"🚀 開始執行地端 FAQ 評估測試，共 {len(FAQ_QUESTIONS)} 個問題。")
    print("💡 測試將強制使用『地端模式 (Ollama/Gemma3)』進行推理，完全不消耗 Gemini API 限額。")
    
    # 確保資料庫已就緒
    db = init_vector_db()
    if not db:
        print("❌ 錯誤：向量資料庫未就緒，請先放入 PDF 檔案。")
        return
        
    results = []
    output_file = "/Users/bensonhong/Desktop/Antigravity專案/管哩資訊系統期末（Benson組)/demo_faq_100.json"
    report_file = "/Users/bensonhong/Desktop/Antigravity專案/管哩資訊系統期末（Benson組)/demo_faq_100.md"
    
    start_time = time.time()
    
    for idx, q in enumerate(FAQ_QUESTIONS, 1):
        print(f"[{idx:03d}/{len(FAQ_QUESTIONS)}] 正在測試問題: {q}")
        q_start = time.time()
        
        try:
            # 傳入 api_key=None 且 disable_expansion=True (使用加速模式檢索，快速本地生成)
            res = query_rag(q, api_key=None, db=db, disable_expansion=True)
    ...
    avg_time = total_time / len(FAQ_QUESTIONS)
    
    with open(report_file, "w", encoding="utf-8") as f:
        f.write("# 🎓 東吳大學學生事務規章 100 題地端 RAG 問答集 (FAQ)\n\n")
        f.write("本問答集由地端大語言模型 (Ollama / Gemma3) 與 Chroma 向量庫在**純本地模式**下檢索生成。\n")
        f.write(f"- **測試問題總數**: {len(FAQ_QUESTIONS)} 題\n")
        f.write(f"- **總執行時間**: {total_time:.2f} 秒 ({total_time/60:.2f} 分鐘)\n")
        f.write(f"- **平均單題生成時間**: {avg_time:.2f} 秒\n")
        f.write("- **使用推論引擎**: 地端模式 🦉\n\n")
        f.write("---\n\n")
        
        for item in results:
            f.write(f"### ❓ 提問 {item['id']}: {item['question']}\n\n")
            f.write(f"**🦉 RAG 解答**:\n{item['answer']}\n\n")
            if item['sources']:
                f.write("**📌 參考來源檔案**:\n")
                for s in item['sources']:
                    f.write(f"- {s}\n")
            f.write(f"\n*⏰ 地端推理時間: {item['time_seconds']} 秒*\n\n")
            f.write("---\n\n")
            
    print(f"🎉 測試全部完成！")
    print(f"📂 成果檔案：")
    print(f"   1. JSON 格式: {output_file}")
    print(f"   2. Markdown 報告: {report_file}")
